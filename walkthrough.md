...
### 10. 投票の開始・終了ステータス管理機能（3状態制御）
* **ファイル**: [app/admin/[eventId]/page.tsx](file:///c:/Users/user/Antigravity/QRBireg/app/admin/[eventId]/page.tsx)
* **ファイル**: [app/my-dashboard/page.tsx](file:///c:/Users/user/Antigravity/QRBireg/app/my-dashboard/page.tsx)
* **ファイル**: [app/vote/page.tsx](file:///c:/Users/user/Antigravity/QRBireg/app/vote/page.tsx)
* **概要**:
  * 管理者画面で投票状況（「開始前」「受付中」「投票終了」）を設定できます。
  * マイページ側の投票ボタンには状態に合わせたコメントを表示してグレーアウトし、直接 `/vote` にアクセスがあった場合もガード画面でブロックします。

### 11. 投票のイベント別利用有無（ON/OFF）設定機能
* **ファイル**: [app/admin/[eventId]/page.tsx](file:///c:/Users/user/Antigravity/QRBireg/app/admin/[eventId]/page.tsx)
* **ファイル**: [app/my-dashboard/page.tsx](file:///c:/Users/user/Antigravity/QRBireg/app/my-dashboard/page.tsx)
* **ファイル**: [app/vote/page.tsx](file:///c:/Users/user/Antigravity/QRBireg/app/vote/page.tsx)
* **概要**:
  * 管理者画面ヘッダーに「投票機能の利用」トグルスイッチを追加しました。
  * 「利用しない」に設定した場合、一般参加者のマイページから「投票」エリア全体が完全に非表示になり、直接 `/vote` にアクセスした際も機能が無効化されている旨が表示されてブロックされます。
  * 同時に、管理者画面の統計ダッシュボードからも投票結果ランキングパネルおよびヘッダーの投票詳細設定（投票枠数や投票状況）が隠れ、すっきりとしたUIになります。

### 12. 投票のイベント別投票基準・説明文の設定機能
* **ファイル**: [app/admin/[eventId]/page.tsx](file:///c:/Users/user/Antigravity/QRBireg/app/admin/[eventId]/page.tsx)
* **ファイル**: [app/my-dashboard/page.tsx](file:///c:/Users/user/Antigravity/QRBireg/app/my-dashboard/page.tsx)
* **ファイル**: [app/vote/page.tsx](file:///c:/Users/user/Antigravity/QRBireg/app/vote/page.tsx)
* **概要**:
  * 管理者画面の統計ダッシュボード内「🏆 投票結果」パネルに、投票基準（説明文）のテキスト入力欄と保存ボタンを追加。DBの `voting_description` をリアルタイムで更新・保存できます。
  * 一般参加者のマイページ上の投票枠、および投票画面（`/vote`）のヘッダー下に、設定された投票基準を「🎯 投票基準」として目立たせて掲示するカードUIを追加しました。
...
* 投票状況を「開始前」「受付中」「投票終了」に切り替えた際、一般マイページのボタンの文言（「投票は開始前です 🔒」「投票は終了しました 🛑」「投票する 🗳️」）が適切に変わり、グレーアウト等の制御が働くことを確認。
...
* 管理者画面ヘッダーから投票数や状況のセレクトボックスが消え、統計タブから「🏆 投票結果」パネルが非表示になることを確認します。
* 一般マイページを開いた際、投票エリアが完全に非表示になっていることを確認します。
* 直接 `/vote` にアクセスした際、「投票は利用できません。投票機能は、このイベントでは無効化されています。」と表示されてブロックされることを確認します。
* 再度管理者画面でトグルを **「利用する」** に切り替えた際、それぞれのエリアや設定項目が通常通り復活することを確認します。

### 5. 投票基準（説明文）の設定と表示の検証
...
* 一般参加者のマイページ（`/[eventId]/my-dashboard`）を開き、投票セクション内の「🎯 投票基準」カードに、上記で設定した説明文が美しく表示されていることを確認します。
